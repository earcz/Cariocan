...
def render(conn, user_row):
    ...
    with c3:
        ...
        if current_act not in act_opts:
            current_act = "Light"
    ...
    if st.button("Save", type="primary"):
        conn.execute("""UPDATE users SET full_name=?, lang=?, avatar=?, email=?, fdc_key=?, plan_type=?, meal_structure=?, 
                     ...
                     (full_name, "en", avatar, email, fdc_key, plan_type, meal_structure,
                      age, sex, height_cm, weight_kg, bodyfat, (bd_input.isoformat() if bd_input else None),
