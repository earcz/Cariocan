...
def render(conn, user_row):
    ...
    with c1:
        ...
        if birthdate:
            try:
                y,m,d = map(int, birthdate.split("-")); bd_val = date(y,m,d)
            except: bd_val = None
    ...
    if st.button("Save", type="primary"):
        conn.execute("""UPDATE users SET full_name=?, lang=?, theme=?, avatar=?, email=?, fdc_key=?, plan_type=?, meal_structure=?, 
                                      age=?, sex=?, height_cm=?, weight_kg=?, bodyfat=?, birthdate=?, activity=?, target_weight=?, training_days=?, fasting=?, waist_cm=? 
                         WHERE username=?""", 
                     (full_name, "en", st.session_state.get("theme","tropical"), avatar, email, fdc_key, plan_type, meal_structure, 
                      age, sex, height_cm, weight_kg, bodyfat, (bd_input.isoformat() if bd_input else None), activity, target_weight, training_days, fasting, waist_cm, st.session_state["user"]))
        conn.commit()
        st.success("Updated")
    ...
    c1.metric("BMR", f"{int(bmr)} kcal"); c2.metric("TDEE", f"{int(tdee)} kcal")
    c3.metric("Workout Day", f"{wcal} kcal"); c4.metric("Rest Day", f"{rcal} kcal")
