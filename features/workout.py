...
def render(conn, user_row):
    ...
    if plan_type=="Push/Pull/Leg":
        schedule = {"Monday":[("Bench Press","4x8","Chest"),("Shoulder Press","3x12","Shoulders")],
        ...
        schedule = {"Monday":[("Bench Press","4x8","Chest"),("Barbell Row","4x8","Back"),("Shoulder Press","3x12","Shoulders")],
        ...
        schedule = {"Monday":[("Treadmill Incline","40min","Cardio"),("Plank","3x max","Core")],
        ...
        schedule = {"Monday":[("Squat","4x8","Quads"),("Bench Press","4x8","Chest"),("Barbell Row","4x10","Back"),("Shoulder Press","3x12","Shoulders")],
        ...
        with cols[0]:
            ...
            if st.button("Save", key=f"save_{i}"):
                conn.execute("""INSERT INTO workout_logs(username, dt, day, exercise, target_sets, target_reps, perf_sets, perf_reps, calories)
                                VALUES(?,?,?,?,?,?,?,?,?)""",
                             (user, date.today().isoformat(), day, name, t_sets, t_reps, int(perf_sets), int(perf_reps), cal))
                conn.commit(); st.success(f"{name} saved (+{cal} kcal)")
            total_burn += cal
        with cols[1]:
            img = muscles.get(mg); 
