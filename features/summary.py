# features/summary.py — fixed safe table creation
...
def render(conn, user_row):
    # --- Tablo güvenliği (eksikse oluştur) ---
    conn.execute("""
    ...
    fig.add_trace(go.Scatter(x=df["dt"], y=df["fat_est"].cumsum(), name="Cum. Fat (kg)", yaxis="y2", mode="lines+markers"))
    fig.update_layout(
